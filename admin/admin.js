...
// Foto: kecilkan dulu (maks 900px, JPEG) agar ringan, lalu upload lewat fungsi server
...
function node(obj, key, title, up) {
  ...
  if (Array.isArray(v)) {
    ...
    if (img) {
      ...
      f.onchange = async () => {
        try {
          say('Mengupload foto…', true);
          const name = 'foto-' + Date.now() + '.jpg';
          const r = await call({ action: 'upload', name, data: await shrink(f.files[0]) });
          obj[key] = inp.value = r.path; say('Foto terupload. Klik Simpan.', true);
...
$('#save').onclick = async () => {
  try { say('Menyimpan…', true); await call({ action: 'save', content: data }); say('Tersimpan ✓ Situs update ±1 menit.', true); }
  catch (e) { say(e.message); }
};
